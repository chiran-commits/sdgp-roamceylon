...
export default function LocationPage() {
  ...
  const [descriptionPlaceholder, setDescriptionPlaceholder] = useState(
    "Enter the features of your ideal location..."
  );
    ...
    const getProfile = async () => {
      ...
      if (accessToken == null) {
        ...
        const res = await axios
          ...
          .then(dispatch(authorizationActions.login()))
          .catch((err) => {
            console.log(err);
            dispatch(authorizationActions.logout());
          });
  ...
  const handleSubmitDescription = async () => {
    ...
    try {
      const locations = await axios.post(
        "https://spacynlp.pythonanywhere.com/nlp",
        ...
        <div className="location-container">
          <div className="leftContainer">
            <div className="input-box-search">
              <input
                ...
                type="text"
                placeholder="Search the location for you...."
                onChange={onChange}
                value={value}
              />
                ...
                <div className="drop-down">
                  {filteredData.map((item) => (
                    <div
                      ...
                      key={item.location}
                      onClick={() => redirectToLocation(item.location)}
                    >
                      <h4>{item.name}</h4>
                    </div>
                  ))}
          ...
          {isLoggedIn && (
            <div className="rightContainer">
              <h2>
                <center>Provide Recommendation</center>
              </h2>
              <div className="textarea-center">
                <textarea
                  placeholder={descriptionPlaceholder}
                  value={descriptionData}
                  className="input-box-recommendation"
                  onChange={handleDescriptionChange}
                ></textarea>
              </div>
              <div className="descriptionKeywords">
                <h4>
                  *Mandatory Keywords: <br></br>
                </h4>
                ...
                <h5 style={{ marginLeft: "10px", color: "darkred" }}>
                  *Cost of Living: 0-50000 (LOW), 50000-200000 (AVERAGE),
                </h5>
                <h5 style={{ marginLeft: "115px", color: "darkred" }}>
                  More than 200000(HIGH).
                </h5>
                <br></br>
                <h5 style={{ marginLeft: "10px", color: "darkred" }}>
                  *WIFI Connectivity : 0-25 (LOW), 25-50 (AVERAGE), More than 50
                  (HIGH).
                </h5>
              </div>
              <button
